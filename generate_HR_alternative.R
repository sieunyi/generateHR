...
generate_HR <- function(L1, L2) {
  ...
  for (i in 1:rounds) {
    ...
    # Use optim() for hr_lpm as well
    result_lpm <- optim(par = 0, fn = lpmObjective,
                        p = p, x_h = x_h, x_l = x_l, y_h = y_h, y_l = y_l,
                        method = "BFGS")
    hr_lpm[i] <- result_lpm$par
...
lpmObjective <- function(h, p, x_h, x_l, y_h, y_l) {
  # xbar <-  (1 - p) * (x_h + h * y_l) + p * (x_l + h * y_h)
  xbar <-0
...
# Now you can access L1 and L2 for each scenario like this:
# L1 for Scenario 1: all_L1_L2$S1$L1
# L2 for Scenario 2: all_L1_L2$S2$L2
# And so on..
